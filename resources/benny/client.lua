...
local locations = {
	[1] = { locked = false, range = 3, outside = { x = -211.614, y = -1324.847, z = 30.890, heading = 270.187}, inside = {x = -211.614, y = -1324.847, z = 30.890, heading = 69.455},job=54,emp=55},
}
			...
			for i, pos in pairs(locations) do
				...
				if distance < pos.range and not near then
					if DoesEntityExist(veh) then
						if not pos.locked and not hasJustBeenInGarage and (User.job == pos.job or User.job == pos.emp) then
							drawTxt("~g~Appuyer sur Enter",4,1,0.5,0.8,1.0,255,255,255,255)
