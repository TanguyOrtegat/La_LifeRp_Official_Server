...
--MENU D'ACCUEIL--
function MenuTrunk()
    MenuTitle = "Menu vehicule"
    Menu.addButton("Mettre un item dans la voiture", "GetPlayerItem", ind)
    Menu.addButton("Sortir un item de la voiture", "GetTrunkItem", ind)
end
...
function GetItem(arg)
    ...
    if vehFront > 0 then
        ...
        if (type(qty) ~= "number") then
            Notify("Ce montant n'est pas valable")
            return false
        end
        if tonumber(qty) <= tonumber(qtymax) and tonumber(qty) > -1  then
            ...
            Notify("Il n'y as pas assez d'item : " .. lib .. " dans la voiture")
        end
    end
    Menu.hidden = true
end
...
function PutItem(arg)
    ...
    if vehFront > 0 then
        ...
        if tonumber(qty) <= tonumber(qtymax) and tonumber(qty) > -1 then
            ...
            Notify("Vous n'avez pas assez d'item : " .. lib .. " sur vous")
        end
    end
    Menu.hidden = true
end
