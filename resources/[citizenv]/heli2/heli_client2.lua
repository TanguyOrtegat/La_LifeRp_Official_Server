...
-- Edited by Flaco and Erkoz with the help of rjross2013
-- Version 1.3 2017-06-12

-- config
local clientside_light = false
local fov_max = 80.0
local fov_min = 10.0 -- max zoom level
...
local toggle_helicam = 38 -- control id of the button by which to toggle the helicam mode. 38 = E
local toggle_vision = 25 -- control id to toggle vision mode. Default: 25 = Right mouse btn (useless here)
local toggle_rappel = 22   -- control id to rappel out of the heli. 22 = SPACE
local toggle_spotlight = 47 -- control id to toggle the front spotlight : 47 = G also change this value at line 82
local toggle_lock_on = 18  -- control id to lock onto a vehicle with the camera. 18 = ENTER
local brightness = 2
-- Script starts here
local helicam = false
local frogger2_hash = GetHashKey("frogger")
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				...
				if IsControlJustPressed(0, 47) then
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					if clientside_light then
						clientside_light = false
					else
						clientside_light = true
					end
				end
				...
				if locked_on_vehicle then
					if DoesEntityExist(locked_on_vehicle) then
						...
						local dists = GetDistanceBetweenCoords(coords1.x, coords1.y, coords1.z, coords2.x, coords2.y, coords2.z, false)
						if dists > 2000 then
							PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
				...
				Citizen.Wait(0)
				if clientside_light then
					local spotoffset = GetEntityCoords(heli)

					local rotation = GetCamRot(cam, 2)
					local forward_vector = RotAnglesToVec(rotation)
					local camcoords = GetCamCoord(cam)
					DrawSpotLightWithShadow(camcoords, forward_vector, 255, 255, 255, 600.0, 10.0, brightness, 10.0, 200.0, 20.0)
				end
			end
			helicam = false
...
function IsPlayerInPolmav()
	...
	return IsVehicleModel(vehicle, frogger2_hash)
end
