...
AddEventHandler("menutaxi:TaxiOG", function(target, rangTaxi)
		if target ~= -1 then
		...
		if rangTaxi == 18 then
			...
			VMenu.AddFunc(98, "Céder la place de patron", "menutaxi:promote", {}, "Accéder")
			VMenu.AddFunc(98, "Engager dans votre entreprise", "menutaxi:hire", {}, "Accéder")
			VMenu.AddFunc(98, "Renvoyer", "menutaxi:fire", {}, "Accéder")
		end
end)
...
function StartDispatch()
    ...
    Wait(2000)
    isDispatch = true
    DrawMissionText("Conduisez aux alentours pour trouver des ~y~clients~w~.", 10000)
end
...
RegisterNetEvent('menutaxi:startDispatch')
AddEventHandler('menutaxi:startDispatch', function(target)
  if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
  if isDispatch == false then
      if IsVehicleModel(GetVehiclePedIsUsing(GetPlayerPed(-1)), GetHashKey("taxia")) then
          StartDispatch()
      else
         TriggerEvent("itinerance:notif", "~r~Vous n'êtes pas dans votre taxi !")
      end
   else
      StopDispatch()
   end
        ...
        if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
          if DoesEntityExist(jobs.peds[1]) then
            if IsPedFatallyInjured(jobs.peds[1]) then
              ...
              DrawMissionText("Votre client est ~r~inconscient~w~. Trouvez un autre client.", 5000)
            else
              if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
                ...
                if jobs.flag[2] == 0 then
                  ...
                  Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
                  jobs.peds[1] = nil
                  DrawMissionText("Votre client est ~r~parti~w~. Trouvez un autre client.", 5000)
                  jobs.flag[1] = 0
                  jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
                else
                  if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
              ...
              if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
                ...
                if jobs.flag[2] == 0 then
                  ...
                  Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
                  jobs.peds[1] = nil
                  DrawMissionText("~r~Votre client n'ira pas avec vous~w~. Trouvez un autre client.", 5000)
                  jobs.flag[1] = 0
                  jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
                else
                  if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
                    ...
                    if street[2] ~= 0 and street[2] ~= nil then
                      local streetname = string.format("~c~Déposez moi à %s, près de %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
                      DrawMissionText(streetname, 5000)
                    else
                      local streetname = string.format("~c~Déposez moi à %s", GetStreetNameFromHashKey(street[1]))
                      DrawMissionText(streetname, 5000)
                    end
              ...
              if jobs.flag[1] == 3 then
                if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), jobs.coords.cx[jobs.flag[2]],jobs.coords.cy[jobs.flag[2]],jobs.coords.cz[jobs.flag[2]], true) > 3.0001 then
                  ...
                  jobs.peds[1] = nil
                  Wait(6000)
                  DrawMissionText("~g~Vous avez deposé le client !", 5000)
                  TriggerServerEvent("menutaxi:paidfordispatch")
                  Wait(8000)
                  DrawMissionText("Conduisez aux alentours pour trouver des ~y~clients~w~.", 10000)
                  jobs.flag[1] = 0
                  jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
            ...
            if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
              ...
              if jobs.flag[2] == 0 then
                ...
                if DoesEntityExist(rped) then
                  ...
                  DrawMissionText("~g~Un client~w~ attend un taxi tout proche de vous.", 5000)
          ...
          if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 30.0001 then
            ...
            DrawMissionText("Remontez dans votre voiture ou arrêtez votre dispatch.", 1)
          end
        end
      else
        StopDispatch()
        DrawMissionText("Votre taxi est ~r~détruit~w~.", 5000)
      end
    end
  end
end)
