...
local playerSource = source
  TriggerEvent('es:getPlayerFromId', netID, function(user)
	...
	if (user) then
		if tonumber(user.job) == 46 or tonumber(user.job) == 45 then
...
AddEventHandler('menucartel:promote_s', function(netID)
	...
	local identifiern = tIdentifiern[1]
		if (tonumber(user.job)) == 46 then
			...
			MySQL.Async.execute("UPDATE users SET `job`=@value WHERE identifier = @identifier", {['@value'] = 46, ['@identifier'] = tostring(identifiern)})
			LaLife.Player.Manager.SetPlayerJob(usern, 46)
